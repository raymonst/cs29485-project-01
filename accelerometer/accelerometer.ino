...
void loop()
{
  ...
  if ((abs((abs(old_gyro_x) - abs(accel_t_gyro.value.x_gyro))) > 1000) ||
      (abs((abs(old_gyro_y) - abs(accel_t_gyro.value.y_gyro))) > 1000) ||
      (abs((abs(old_gyro_z) - abs(accel_t_gyro.value.z_gyro))) > 1000)) {
    myservo.write(180);
    delay(750);
    myservo.write(0);
    delay(750);
